...
export interface GetWpV2CustomTaxonomyRequest {
    name: string;
}
...
export class DefaultApi extends runtime.BaseAPI {
    ...
    /**
     */
    async getWpV2CustomTaxonomyRaw(requestParameters: GetWpV2CustomTaxonomyRequest): Promise<runtime.ApiResponse<Array<CustomTaxonomy>>> {
        if (requestParameters.name === null || requestParameters.name === undefined) {
        ...
        const response = await this.request({
            path: `/wp/v2/{name}`.replace(`{${"name"}}`, encodeURIComponent(String(requestParameters.name))),
            method: 'GET',
            headers: headerParameters,
            query: queryParameters,
        });

        return new runtime.JSONApiResponse(response, (jsonValue) => jsonValue.map(CustomTaxonomyFromJSON));
    }
